...
class CoinBYBITCONTRACT:
    ...
    async def get_price(self):
        ...
        def fetch_price_sync(coin):
            try:
                ...
                if ticker_info and ticker_info['result']['list'][0][4]:
                    logging.info(f"BYBIT永续合约 ticker_info_data: {ticker_info}")
                    ...
                    logging.info(f"BYBIT永续合约 解析 {coin} 的价格失败")
            except Exception as e:
                logging.info(f"BYBIT永续合约 获取 {coin} 的价格失败，错误: {e}")
        ...
        logging.info(f"BYBIT永续合约 价格数据: {price_info}")
        return price_info
    ...
    async def create_order(self, symbol, num, price):
        ...
        def place_order_sync():
            """同步下单函数"""
            try:
                setleverage = self.session.set_leverage(
                    category="linear",
                ...
                if setleverage:
                    if setleverage['retCode'] == 0:
                        logging.info(f"BYBIT永续合约 设置 {symbol} 杠杆2x")
                        ...
                        logging.error(f"BYBIT永续合约 设置 {symbol} 杠杆失败: {setleverage}")
                else:
                    logging.error(f"BYBIT永续合约 设置 {symbol} 杠杆失败 setleverage无响应")
            except Exception as e:
                if "110043" in str(e):
                    logging.warning(f"BYBIT永续合约 设置 {symbol} 杠杆未修改 (ErrCode: 110043)，继续下单")
                    return self.session.place_order(
                        category="linear",
                        symbol=f"{symbol}USDT",
                        side="Buy",
                    ...
                    logging.info(f"BYBIT永续合约 订单创建失败: {symbol}, 错误类型: {type(e).__name__}, 错误详情: {e}")
                return None
        ...
        if order_response:
            if order_response['retCode'] == 0:
                msg += f"BYBIT永续合约 订单创建成功: {symbol}, 数量: {num}, 价格: {price}"
                logging.info(f"BYBIT永续合约 订单响应: {order_response}")
            else:
                msg += f"BYBIT永续合约 订单创建失败: {symbol}, 数量: {num}, 价格: {price}"
                logging.info(f"BYBIT永续合约 订单响应: {order_response}")
        else:
            msg += f"BYBIT永续合约 订单创建失败: {symbol}, 数量: {num}, 价格: {price}"
            logging.info(f"BYBIT永续合约 订单响应: {order_response}")
    ...
    async def spot(self):
        ...
        try:
            ...
            if price_info:
                ...
                logging.info("BYBIT永续合约 error: un_coin")
                return "BYBIT永续合约 订单创建失败"
        except Exception as e:
            logging.info(f"BYBIT永续合约 订单创建失败，错误: {e}")
            return "BYBIT永续合约 订单创建: 现货主逻辑下单错误"
